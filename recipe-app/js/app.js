...
modalText.innerHTML = `<h3>${data.strMeal}</h3> <br/> <h4>${data.strCategory}</h4> <br/> <h5>Instructions:</h5> <br/> <p>${data.strInstructions}</p>` 

let modalImgBox = document.createElement('div')
modalImgBox.className = 'modal-imgbox'
modalImgBox.innerHTML = `<img src=${data.strMealThumb} class ='modal-img' alt='thumbnailImg'> <br/> <h5>Watch Video</h5>`
...
watchvideo[0].addEventListener('click', (event) => {
 let videoFrame = document.createElement('div')
 videoFrame.className = 'video-frame'
 videoFrame.innerHTML = `<video src="${data.strYoutube}" controls></video>` // corb오류
})
}
/* 모달윈도우 생성 마운트 함수 - 끝 */
...
const foodName = document.querySelector('.food-text h3')
    ...
    for (const btn of modalOpenBtn){
        ...
        // console.log(e.target.parentElement.parentElement) // 디버깅용
       createModal(defalutReults)
        //   모달닫기 버튼 기능
